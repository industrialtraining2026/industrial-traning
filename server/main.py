...
# These routes should be last to avoid conflicting with API routes
@app.get("/")
async def read_root():
...
async def serve_frontend(path: str):
    ...
    if file_path.exists() and file_path.is_file():
        ...
        # For files without extension (like components/), check if it's a JS file
        if file_path.suffix == '' and file_path.exists():
            # Try to serve as JS if no extension
            try:
